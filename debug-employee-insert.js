...
async function ensureDependencies() {
  console.log('\n🔗 Checking and creating required dependencies...');
  ...
  try {
    ...
    if (!orgExists) {
      ...
      console.log(`✅ Organization exists: ${orgExists.name}`);
    }
    ...
    if (!deptExists) {
      ...
      console.log(`✅ Department exists: ${deptExists.name}`);
    }
    ...
    if (!empTypeExists) {
      ...
      console.log('✅ Employment Type created successfully');
    } else {
      console.log(`✅ Employment Type exists: ${empTypeExists.name}`);
    }
    
    // 4. Check/Create Work Location
    const workLocExists = await prisma.workLocation.findUnique({
      where: { work_location_id: testEmployee.work_location_id }
    });
    
    if (!workLocExists) {
      console.log(`Creating work location with ID: ${testEmployee.work_location_id}`);
      await prisma.workLocation.create({
        data: {
          work_location_id: testEmployee.work_location_id,
          org_id: testEmployee.org_id,
      ...
      console.log('✅ Work Location created successfully');
    } else {
      console.log(`✅ Work Location exists: ${workLocExists.name}`);
    }
    ...
    if (!jobTitleExists) {
      ...
      console.log(`✅ Job Title exists: ${jobTitleExists.name}`);
    }
    
    console.log('\n✅ All dependencies created/verified successfully');
    return true;
  } catch (error) {
...
async function testEmployeeAPI() {
  console.log('\n📋 EMPLOYEE API TEST - DEBUGGING SESSION');
  console.log('======================================');
  
  // Step 1: Ensure all dependencies exist
  const dependenciesOk = await ensureDependencies();
  if (!dependenciesOk) {
    console.error('❌ Cannot proceed with employee creation due to dependency issues');
    return;
  }
  ...
  try {
    ...
    console.log('\n✅ TEST PASSED: Employee created and retrieved successfully!');
  } catch (error) {
    console.error('\n❌ Error occurred:');
    if (error.response) {
